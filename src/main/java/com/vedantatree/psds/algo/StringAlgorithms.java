 ...
 * 
 * Rotate Matrix
 * Replace row, column in matrix to zero
 * 
...
public class StringAlgorithms extends TestCase
{
	 ...
	 * 
	 * concept is >
	 * check if we have all the characters in even number, if yes, that means these characters can be repeated twice or
	 * in pair
	 * if there is any character counting odd, it can not be more than once. I.e. in the center of the palindrome.
	...
	/**
	 * Map each character to a number. Like a -> 0, b -> 1, c -> 2, etc.
	 ...
	 * @param trueLength actual length of string which contains the content, leaving trailing spaces for %20
	 ...
	 * @param trueStringLength Given true length of string, excluding trailing spaces if any (trailing space to
	 *        accommodate urlify logic)
	...
	public boolean isPermutationOptimized( String string1, String string2 )
	{
		 ...
		 * check for each character in array, if it is found > reduce count by 1.
		 * if remainder count is less than zero, it means count from first string were less than second string > return
		 * false
		 */
	...
	/**
	 * @return true if both string are same, or both string can be same by replacing/adding one of the character in
	 *         either
	 */
