...
public class TreeAlgo {
	...
	private static int[] getDiameterOfBinaryTreeInternal( XTreeNode<Integer> node, int maxDiameter ) {
		...
		if( node.getLeftChild() != null ) {
			leftValues = getDiameterOfBinaryTreeInternal( node.getLeftChild(), maxDiameter );
		}

		int[] rightValues = null;
		if( node.getRightChild() != null ) {
			rightValues = getDiameterOfBinaryTreeInternal( node.getRightChild(), maxDiameter );
		}
		...
		int currentNodeDiameter = leftCount + rightCount;

		maxDiameter = currentNodeDiameter > maxDiameter ? currentNodeDiameter : maxDiameter;
	...
	/**
	 * @param tree tree to check for height balance
	...
	private static int countHeightDifference( BinaryTree rootNode ) {
		if( rootNode == null )
			return 0;

		int left = countHeightDifference( rootNode.left );
		int right = countHeightDifference( rootNode.right );
