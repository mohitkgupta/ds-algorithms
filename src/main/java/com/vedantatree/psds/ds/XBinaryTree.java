...
public class XBinaryTree<E extends Comparable<E>>
{
	 ...
	 * Ensure that hight should be minimum
	 * 
	 * Algo:
	 * Hight can be optimum only if both left and right are properly utilized
	 * which can be done by picking center value of the array and then creating left, right node from left right
	 * subsection of array in similar fashion
	 * 
	 ...
	 * 
	 * Algo:
	 * If given data is smaller than root node > call to add on left side recursively
	 * If given data is bigger than root node > call to add on right side recursively
	 * 
	...
	public XTreeNode<E> findCommonAncestorWithoutParentLink( E node1Data, E node2Data )
	{
		...
		// if both data are same, it means both are referring to same node. Return first node found as intersection
	...
	public XTreeNode<E> findCommonAncestor( E node1Data, E node2Data )
	{
		 ...
		 * get delta
		 * For deeper node, go up by delta, so we have both node at same depth
		 * Now start iterating towards parent node and keep comparing the nodes
		...
		// if any of the node become node, which means these are not related to same tree - return null, which means no
		// intersection
	 ...
	 * Return value can be null too if specified delta is more than node's own depth
	 */
	...
	/**
	 * Structure to contain result of tree traversal and more information like BST check if applicable
	 * 
	...
	/*
	 * Algo:
	 * In BST, whole tree on the left should be smaller than root node, and should be larger than root node on the right
	 * This concept applies to each node in tree
	 * While traversing, we can pass root node value down to compare whether child node is smaller if it is left, or
	 * larger if it is right
	 */
	 ...
	 * When is a BST balanced - when both left and right node should not have height difference of more than one
	 * Check this recursively
	 * If difference is more, return error code
	 * 
	 * @return Integer.MIN_VALUE as error code if tree is not balance, tree height otherwise
	 */
	...
	/**
	 * Print tree in Matrix form
	 * Most readable form
	 * 
	 * However, alignment gets disturbed as numbers get bigger
	 * We can fix it by filling max space in empty place, and equal width for number,
	 * but that will make the logic bit complex.
	 * 00Hence leaving for now
	 */
